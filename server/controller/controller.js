...
exports.create = (req,res)=>{
    ...
    user
     ...
     .catch(err =>{
         res.status(500).send({
             message:err.message || "Some error occured while creating a new create operation"
         })
     })
}
...
exports.find = (req,res)=>{
    if(req.query.id){
        ...
        .catch(err=>{
            res.status(500).send({message:err.message || "Error occured while retriving a user information"})
        })
    }
}
